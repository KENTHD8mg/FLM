...
- 神经网络
	...
	- ResNet
	- 反向传播公式**
		- $$\begin{align}&W^{[l]}表示第l层权值,B^{[l]}表示第l层偏置,Z^{[l]}表示l层输出,A^{[l]}表示第l层激活值,G^{[l]}表示第l层激活函数，有\\&dW^{[l]}=\frac{dZ^{[l]}}{(A^{[l-1]})}\\&dB^{[l]}=np.sum(dZ^{[l]},axis=1,keepdim=True)\\&dZ^{[l]}=\frac{dZ^{[l+1]}}{W^{[l+1]}(dG^{[l]})(Z^{[l]})}\end{align}$$
- SVM
- 决策树
- 随机森林
- Adaboost
